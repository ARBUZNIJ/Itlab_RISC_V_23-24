...
public:
	...
	T scl_n(T** data_1, size_t ind_1_n, T** data_2, size_t ind_2_n)  //функци€ скал€рного произведени€ дл€ столбцов двух матриц одинакового размера
	{
		T res=0;
	...
	void norm()
	{
		...
		{
			tmp = sqrt(scl_sqr_n(Q, j));       //каким образом можно распознать нули, не ввод€ формальной проверки?
			if (tmp <= eps)
				tmp = 0;
			else tmp = 1 / tmp;
			for (i = 0; i < n; i++)
	...
	void form_R()
	{
		...
		for (i = 0; i < n; i++)
		{
			...
			for (j = 0; j < n; j++)
				R[i][j] = 0;
			//memset(Q[i], 0, n);            //почему неправильно?
		}
		for (i = 0; i < n; i++)
			for (k = 0; k < n; k++)
				for (j = 0; j < n; j++)
